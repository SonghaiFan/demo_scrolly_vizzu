export class StoryBuilder {
  ...
  addStep(content, options = {}) {
    if (!this.currentChapter) {
      throw new Error('Cannot add step without a chapter. Call chapter() first.');
    }
  ...
  generateDOM() {
    ...
    this.chapters.forEach((chapter, chapterIndex) => {
      ...
      chapterEl.setAttribute('data-scrollama-index', this.getStepIndex(chapterIndex));
      ...
      if (chapter.steps && chapter.steps.length > 0) {
        ...
        chapter.steps.forEach((step, stepIndex) => {
          ...
          stepEl.setAttribute('data-scrollama-index', this.getStepIndex(chapterIndex, stepIndex));
